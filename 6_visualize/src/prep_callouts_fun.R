  ...
  if(n_callouts_text > 0) {
    ...
    plot_fun <- function(){
      ...
      for(n in 1:n_callouts_text) {
        ...
        if(callout_text_cfg_n$cex < 4 | callout_text_cfg_n$cex > 8) {
          stop("Unsupported `cex` used. Should be between 4 and 8 for best spacing. Otherwise, new spacing needs to be tested with `cex` or wonky spacing will be in final image. See `prep_callouts_fun`")
          font_x_multiplier <- 1
          font_y_multiplier <- 1
        } else {
          font_x_multiplier <- approx(c(4,8), c(4.1, 8.2), callout_text_cfg_n$cex)$y # for Abel
          font_y_multiplier <- approx(c(4,8), c(5, 11.5), callout_text_cfg_n$cex)$y # for Abel
        }
