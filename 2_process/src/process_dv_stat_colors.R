...
process_dv_stat_colors <- function(ind_file, dv_stats_ind, color_palette, gage_style){
  ...
  dv_stats_with_color <- dv_stats %>%
    mutate(shape = ifelse(is.na(per), no = gage_style$with_percentile$pch,
                         yes = gage_style$no_percentile$pch),
          size = ifelse(is.na(per), no = gage_style$with_percentile$cex,
  ...
  dv_stats_with_color$color[!per_na] <- rgb(col_fun(dv_stats$per[!per_na]), maxColorValue = 255)
